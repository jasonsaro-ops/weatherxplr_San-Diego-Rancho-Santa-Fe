...
const localLat = 33.0239;
const localLon = -117.2000;
let countdownVal = 120;

const AIRNOW_API_KEY = "E5AFEF36-80F6-4A42-AE38-F3C56E3AEAC4"; 

const sanDiegoGauges = [
    { id: "330225117092701", name: "San Dieguito River Near Rancho Santa Fe, CA", lat: 33.0403, lon: -117.1575, noaaId: "SDCA1" },
    { id: "11030500", name: "San Dieguito River Near Del Mar, CA", lat: 32.9775, lon: -117.2297, noaaId: "SDCA2" },
    { id: "11023340", name: "Los Peñasquitos Creek Near Poway, CA", lat: 32.9431, lon: -117.1217, noaaId: "SDCA3" }
];
        ...
        content: [
            {
                ...
                content: [
                    ...
                    { type: 'component', componentName: 'localForecast', title: '7-DAY GEOGRAPHIC SYNOPTIC OUTLOOK (92067)' }
                ]
            },
            {
                type: 'column',
                width: 30,
                content: [
                    { type: 'component', componentName: 'nwsAlerts', title: 'CRITICAL ENVIRONMENTAL SPECTRUM HAZARDS MATRIX - CALIFORNIA' },
                    { type: 'component', componentName: 'hydrologyFeed', title: 'SAN DIEGUITO / LOS PEÑASQUITOS WATERSHED REAL-TIME STREAMFLOW' }
                ]
            },
            {
                type: 'column',
                ...
                content: [
                    ...
                    { type: 'component', componentName: 'noaaTides', title: 'NOAA TIDES & CURRENTS (STATION 9410230 - LA JOLLA)' }
                ]
            }
        ]
    }]
...
layout.registerComponent('radarMap', function(container) {
    container.getElement().html(`
        <div style="position:relative; width:100%; height:100%; background:#0d1117;">
            ...
            <iframe id="windyIframe" src="https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=in&metricTemp=f&metricWind=mph&zoom=8&overlay=radar&product=radar&level=surface&lat=33.0239&lon=-117.2000" style="width:100%; height:100%; border:none;"></iframe>
        </div>
    `);
    ...
    setTimeout(() => {
        ...
        select.on('change', function() {
            ...
            iframe.src = `https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=in&metricTemp=f&metricWind=mph&zoom=8&overlay=${layer}&product=${product}&level=surface&lat=33.0239&lon=-117.2000`;
        });
    }, 200);
});
...
layout.registerComponent('cloudMap', function(container) {
    container.getElement().html(`
        <div style="position:relative; width:100%; height:100%; background:#0d1117;">
            ...
            <iframe id="windyCloudIframe" src="https://embed.windy.com/embed.html?type=map&location=coordinates&metricRain=in&metricTemp=f&metricWind=mph&zoom=8&overlay=clouds&product=gfs&level=surface&lat=33.0239&lon=-117.2000" style="width:100%; height:100%; border:none;"></iframe>
        </div>
    `);
...
layout.registerComponent('nwsAlerts', function(container) {
    ...
    container.on('open', fetchCaliforniaAlerts);
});
...
layout.registerComponent('hydrologyFeed', function(container) {
    ...
    container.on('open', fetchSanDiegoHydrology);
});
...
function fetchCaliforniaAlerts() {
    const container = $('#alerts-container');
    console.log("Starting California alerts fetch...");
    
    // Use the NWS alerts API with a broad California search
    // Query all alerts and filter for CA
    fetch('https://api.weather.gov/alerts/active')
        ...
        .then(data => {
            ...
            // Filter for California alerts only
            let caAlerts = allAlerts.filter(f => {
                const areaDesc = f.properties.areaDesc || '';
                const properties = f.properties;
                
                // Check if alert applies to CA
                return areaDesc.includes('CA') || 
                       areaDesc.includes('California') ||
                       (properties.areaDesc && properties.areaDesc.toLowerCase().includes('california'));
            });
            
            console.log("California alerts found:", caAlerts.length);
            caAlerts.forEach(alert => {
            ...
            if (caAlerts.length > 0) {
                html += `<div style="margin-bottom:10px; padding-bottom:8px; border-bottom:1px solid #30363d;">
                    <div style="font-size:0.75rem; color:#ffcc00; font-weight:bold; margin-bottom:5px;">ACTIVE ALERTS - ${caAlerts.length}</div>`;
                ...
                caAlerts.forEach(f => {
                    ...
                    const criticalEvents = ['Tornado Warning', 'Severe Thunderstorm Warning', 'Flash Flood Warning', 'Winter Storm Warning', 'Flood Warning', 'Hurricane Warning', 'Red Flag Warning', 'Fire Warning', 'Extreme Fire Danger'];
                ...
                html = "<span style='color:#00ff55; font-size:0.8rem;'>✓ SYSTEM CLEAN: NO ACTIVE ALERTS FOR CALIFORNIA</span>";
            }
        ...
        .catch(err => {
            console.error("California alerts fetch error:", err);
...
function openAlertDetails(id) {
    ...
    openFloatingModal(`CALIFORNIA ALERT DETAILS`, body);
}
            ...
            let html = `
                <div style="margin-bottom:8px; padding-bottom:6px; border-bottom:1px dashed #30363d;">
                    <div style="font-size:0.85rem; color:#fff;">
                        <i class="fa-solid fa-satellite-dish"></i> RANCHO SANTA FE, CA (92067)
...
function fetchNOAATides() {
    // NOTE: 9410230 (La Jolla, CA - Scripps Institution Pier) is the nearest
    // active NOAA NWLON station to the Rancho Santa Fe / Solana Beach coastline
    // (~9 miles from Rancho Santa Fe's nearest ocean access). Unlike some
    // inlet-adjacent subordinate stations, La Jolla is a full harmonic station
    // with live water level and meteorological sensors, so both predictions
    // and observed readings are pulled from it directly.
...
function fetchSanDiegoHydrology() {
...
setInterval(() => {
    ...
    if(countdownVal <= 0) {
        countdownVal = 120;
        fetchNWSForecast();
        fetchCaliforniaAlerts();
        fetchAirQualityData();
        fetchNOAATides();
        fetchSanDiegoHydrology();
    }
